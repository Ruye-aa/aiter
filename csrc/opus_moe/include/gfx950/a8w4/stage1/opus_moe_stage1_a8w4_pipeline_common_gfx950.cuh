...
struct Tile
{
    int tid, lane_id, wave_id, route_base, out_col_base, out_half, group_base;
};
...
{
    return payload_base +
           static_cast<int64_t>(item * Traits::MFMA_K_STEPS + k_step) *
...
__device__ __forceinline__ int safe_a_scale_word(
    ...
    const int (&a_scale)[Traits::M_SCALE_PACKS])
{
    ...
    const bool scale_route_valid =
        ...
        a_route_valid[Mi];
    return scale_route_valid ? a_scale[Mi / Traits::SCALE_MN_PACK] :
                               kNoScaleWord;
}

template<typename Traits, int Mi, int Kk>
__device__ __forceinline__ int select_sfa_stage1(
    const bool (&a_route_valid)[Traits::M_MFMA_PER_WAVE],
    const int (&a_scale)[Traits::M_SCALE_PACKS])
{
    return safe_a_scale_word<Traits, Mi>(a_route_valid, a_scale);
}

template<typename Traits, int LocalGroup, int Item, int Kk>
__device__ __forceinline__ int select_sfb_stage1(
    const int (&b_scale)[Traits::B_GROUPS_PER_WAVE])
{
    return b_scale[LocalGroup];
}
...
__device__ __forceinline__ int a_lane_byte_stage1(
    const ALayout& u_ga, int mi)
{
    const int a_lane_base = static_cast<int>(u_ga(0, 0, 0, 0));
    return mi * Traits::MMA_M * Traits::H + a_lane_base;
}

template<typename Traits>
__device__ __forceinline__ int a_lane_local_m_stage1(int a_lane_byte)
{
    return a_lane_byte / Traits::H;
}

template<typename Traits>
__device__ __forceinline__ int a_lane_k_byte_stage1(int a_lane_byte)
{
    return a_lane_byte - a_lane_local_m_stage1<Traits>(a_lane_byte) *
                             Traits::H;
}
    ...
    int b_lane_byte)
{
    return static_cast<int64_t>(expert_id) * kargs.stride_w1_e +
           static_cast<int64_t>(w1_n0) * 2 * Traits::MFMA_K_STEPS *
               Traits::W1_PAYLOAD_K_STEP_STRIDE_BYTES +
           b_lane_byte;
}
...
__device__ __forceinline__ int b_scale_mn_pack_stage1(
    int expert_id, int w1_n0)
{
    return expert_id * Traits::GATE_UP_LOGICAL_DIM /
...
__device__ __forceinline__ int b_scale_base_word_stage1(
    const ScaleLayout& u_sf, int expert_id, int w1_n0)
{
    return scale_base_word_stage1(
        u_sf, b_scale_mn_pack_stage1<Traits>(expert_id, w1_n0));
}
    ...
    stage1_u32x4_t (&b_kk0)[Traits::B_ITEMS_PER_WAVE])
{
    ...
    for(int flat_item = 0; flat_item < Traits::B_ITEMS_PER_WAVE; ++flat_item)
    {
        ...
        const int64_t b_offset =
            b_payload_offset<Traits>(
                b_group_payload_base[local_group], item, k_pair);
        b_kk0[flat_item] = __builtin_bit_cast(
            stage1_u32x4_t,
    ...
    const int64_t (&a_payload_base)[Traits::M_MFMA_PER_WAVE])
{
    ...
    opus::static_for<Traits::M_MFMA_PER_WAVE>([&](auto mi_id) {
        ...
        for(int kk = 0; kk < Traits::SCALE_K_PACK; ++kk)
        {
            ...
            {
                const auto a_lo =
                    hidden_gmem.template load<Traits::BYTES_PER_VEC>(
                        static_cast<int>(a_offset));
                const auto a_hi =
                    hidden_gmem.template load<Traits::BYTES_PER_VEC>(
                        static_cast<int>(
                            a_offset + 4 * Traits::BYTES_PER_VEC));
                s_a.template store<Traits::BYTES_PER_VEC>(a_lo, smem_lo);
                s_a.template store<Traits::BYTES_PER_VEC>(a_hi, smem_hi);
            }
        }
    });
}
...
template<typename Traits, typename HiddenGmem, typename SmemA, typename SmemLayout>
__device__ __forceinline__ void prefetch_next_a_stage1(
    HiddenGmem& g_a,
    SmemA& s_a,
    const SmemLayout& u_sa,
    const Tile& tile,
    ...
    const int64_t (&a_payload_base)[Traits::M_MFMA_PER_WAVE])
{
    if(next_k_pair < Traits::MFMA_K_STEPS)
    {
        stage_a_reg_kpair_to_lds<Traits>(
            g_a, s_a, u_sa, tile, next_k_pair,
            a_route_valid, a_payload_base);
    }
}

template<typename Traits>
__device__ __forceinline__ void wait_next_a_stage1(int next_k_pair)
{
    if(next_k_pair < Traits::MFMA_K_STEPS)
    {
        if constexpr(Traits::ASYNC_A_REG_LDS)
...
__device__ __forceinline__ void epilogue_store_group_to_smem(
    ...
    const Tile& tile, int group, int row_pass_base)
{
    opus::static_for<4>([&](auto ii_id) {
        ...
        const float value =
            silu_mul(static_cast<float>(gate_or_acc[ii]), static_cast<float>(up[ii]));
    ...
    opus::static_for<kMiPerPass>([&](auto mi_local_id) {
        ...
        opus::static_for<kStoreGroups>([&](auto local_group_id) {
            ...
            epilogue_store_group_to_smem<Traits, CAcc>(
                smem_values, rc[mi][local_group],
                rc[mi][local_group + kUpOffset], u_c_m, u_c_n,
                tile, group, kRowPassBase - mi * Traits::MMA_M);
        });
    });
}
    ...
    float* __restrict__ smem_gate_up)
{
    opus::static_for<Traits::EPILOGUE_ROW_SPLIT>([&](auto row_pass_id) {
        ...
        epilogue_store_row_pass<Traits, row_pass, CAcc>(
            smem_gate_up, rc, u_c_m, u_c_n, tile);
        opus::sync_threads();
...
inline __device__ auto make_layout_ga_stage1(Mma& mma, int lane_id)
{
    ...
    return opus::partition_layout_a<T::BYTES_PER_VEC>(
        mma,
        opus::make_tuple(opus::number<T::H>{}, opus::number<1>{}),
        p_coord_a);
}
...
inline __device__ auto make_layout_scale_word_stage1(int scale_lane_byte)
{
    ...
    return opus::make_layout(
        ...
        opus::unfold_x_stride(
            block_dim,
            block_shape,
            opus::tuple{opus::number<T::SCALE_LAYOUT_STRIDE_N0>{},
                        opus::number<T::MMA_M>{},
                        opus::number<1>{}}),
        opus::unfold_p_coord(
            block_dim,
