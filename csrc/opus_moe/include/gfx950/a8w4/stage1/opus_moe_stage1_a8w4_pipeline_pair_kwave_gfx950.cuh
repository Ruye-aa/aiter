...
{
    static_assert(!T::GATE_UP_GROUP_SPLIT && T::PAIR_GATE_UP_SINGLE_GROUP,
    ...
    if(route_base >= valid_rows || expert_id < 0 || expert_id >= T::EXPERTS)
        return;
    ...
    auto u_ga = make_layout_ga_stage1<T>(mma, lane_id);
    ...
    auto u_sf = make_layout_scale_word_stage1<T>(b_lane_byte);
    ...
    {
        const int a_lane_byte = a_lane_byte_stage1<T>(u_ga, mi);
        const int local_m = a_lane_local_m_stage1<T>(a_lane_byte);
        const int k_byte = a_lane_k_byte_stage1<T>(a_lane_byte);
        ...
        ++local_group)
    {
        ...
        b_group_payload_base[local_group] =
            b_payload_base_stage1<T>(
                kargs, expert_id, w1_n0, b_lane_byte);
        b_scale_base_word[local_group] =
            b_scale_base_word_stage1<T>(u_sf, expert_id, w1_n0);
    }
    ...
    const int k_begin = mainloop_wave_k * T::K_STEPS_PER_WAVE;
    const int k_end = k_begin + T::K_STEPS_PER_WAVE;
        ...
        k_pair += T::SCALE_K_PACK)
    {
        ...
        load_b_kpair_stage1<T>(
            g_b, b_group_payload_base, k_pair, b_kk0);
        ...
        opus::static_for<T::SCALE_K_PACK>([&](auto kk_id) {
            ...
            opus::static_for<T::M_MFMA_PER_WAVE>([&](auto mi_id) {
                ...
                const int64_t a_offset =
                    a_payload_offset<T>(a_payload_base[mi], k_step);
                const auto a_lo = __builtin_bit_cast(
                    stage1_u32x4_t,
                    g_a.template load<T::BYTES_PER_VEC>(
                        static_cast<int>(a_offset)));
                const auto a_hi = __builtin_bit_cast(
                    stage1_u32x4_t,
                    g_a.template load<T::BYTES_PER_VEC>(
                        static_cast<int>(
                            a_offset + 4 * T::BYTES_PER_VEC)));

                ra[mi] = make_a_reg<V_A>(a_lo, a_hi);
            });
            ...
            opus::static_for<T::B_ITEMS_PER_WAVE>([&](auto flat_id) {
                ...
                if constexpr(kk != 0)
                {
                    const int64_t b_offset =
                        b_payload_offset<T>(
                            b_group_payload_base[local_group], item, k_step);
                    b_raw = __builtin_bit_cast(
                        stage1_u32x4_t,
                        g_b.template load<T::BYTES_PER_VEC>(
                ...
                const int sfb =
                    select_sfb_stage1<T, local_group, item, kk>(b_scale);

                opus::static_for<T::M_MFMA_PER_WAVE>([&](auto mi_id) {
                    constexpr int mi = mi_id.value;
                    const int sfa =
                        select_sfa_stage1<T, mi, kk>(
    ...
    reduce_single_group_kwave<T>(tile.wave_id, tile.lane_id,
                                 smem_scratch, v_c);

    quant_epilogue<T>(
        kargs, g_out, g_out_scale, tile, u_c_m, u_c_n, v_c, smem_route,
