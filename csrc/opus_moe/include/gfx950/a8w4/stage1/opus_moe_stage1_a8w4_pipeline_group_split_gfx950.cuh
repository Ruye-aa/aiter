    ...
    int* __restrict__ smem_route)
{
    ...
    stage_a_reg_kpair_to_lds<T>(
        g_a, s_a, u_sa, tile, 0, a_route_valid, a_payload_base);
    if constexpr(T::ASYNC_A_REG_LDS)
        opus::s_waitcnt_vmcnt(opus::number<0>{});
    opus::sync_threads();

    constexpr int k_begin = 0;
    constexpr int k_end = T::MFMA_K_STEPS;
        ...
        k_pair += T::SCALE_K_PACK)
    {
        ...
        opus::static_for<T::SCALE_K_PACK>([&](auto kk_id) {
            ...
            opus::static_for<T::M_MFMA_PER_WAVE>([&](auto mi_id) {
                ...
                const int smem_base =
                    a_reg_lds_load_byte_offset<T>(k_step, mi);
                ra[mi] = __builtin_bit_cast(
                    V_A,
                    opus::load<T::BYTES_PER_VEC>(
                        s_a, u_ra + smem_base));
            });
        ...
        wait_next_a_stage1<T>(next_k_pair);
    }
